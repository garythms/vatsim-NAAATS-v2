...
bool CUtils::GetAircraftDirection(int heading) {
	if ((heading <= 359) && (heading >= 180)) {
		return true; // Westbound
	}
	else if ((heading >= 0) && (heading <= 179)) {
		return false; // Eastbound
	}
	return true;  // Default - treat as westbound
}

bool CUtils::GetDirectionFromADEP(string adep, string ades) {
	if (adep.length() < 2 || ades.length() < 2) return true; // Default West

	// Common European/East prefixes (ICAO)
	string eastPrefixes[] = { "EB", "ED", "EE", "EF", "EG", "EH", "EI", "EK", "EL", "EN", "EP", "ES", "ET", "EU", "EV", "EY", "LF", "LS", "LO", "LH", "LI", "LP", "LE", "GC" };
	// Common North American/West prefixes
	string westPrefixes[] = { "C", "K", "M", "P", "T", "S", "B", "F" };

	bool depEast = false;
	for (const string& p : eastPrefixes) if (adep.substr(0, 2) == p) { depEast = true; break; }
	bool destEast = false;
	for (const string& p : eastPrefixes) if (ades.substr(0, 2) == p) { destEast = true; break; }

	bool depWest = false;
	for (const string& p : westPrefixes) if (adep.substr(0, 1) == p) { depWest = true; break; }
	bool destWest = false;
	for (const string& p : westPrefixes) if (ades.substr(0, 1) == p) { destWest = true; break; }

	if (depEast && destWest) return true; // Westbound
	if (depWest && destEast) return false; // Eastbound

	// Fallback for mid-Atlantic/other cases
	if (depEast && !destEast) return true; // Likely Westbound
	if (!depEast && destEast) return false; // Likely Eastbound

	return true; // Default Westbound
}

bool CUtils::IsEntryPoint(string pointName, bool isWestbound) {
	if (isWestbound) { // Shanwick
...
bool CUtils::IsExitPoint(string pointName, bool isWestbound) {
	if (isWestbound) { // Gander
		if (find(pointsGander.begin(), pointsGander.end(), pointName) != pointsGander.end()) {
			return true; // Match
		}
		else {
...
bool CUtils::IsAircraftRelevant(CRadarScreen* screen, CRadarTarget* target, bool filtersDisabled) {
	...
	if (!filtersDisabled) {
		...
		int flVal = target->GetPosition().GetPressureAltitude();
		int lowFilterValue = AltFiltLow > 0 ? AltFiltLow * 100 : 20000;
		int highFilterValue = AltFiltHigh > 0 ? AltFiltHigh * 100 : 60000;

		if (flVal < lowFilterValue || flVal > highFilterValue) {
			// Undeparted check
			if (flVal < 5000 && gs < 100) return false;
			if (flVal != 0) return false;
		}

		// Use the more robust route-based oceanic check from RoutesHelper
		if (!CRoutesHelper::IsOceanicRoute(screen, fp)) {
			return false;
		}

		// Filter out domestic traffic (e.g. within Europe/UK/Ireland)
		string dep = fp.GetFlightPlanData().GetOrigin();
		string dest = fp.GetFlightPlanData().GetDestination();
		if (dep.size() >= 2 && dest.size() >= 2) {
			string depRegion = dep.substr(0, 2);
			string destRegion = dest.substr(0, 2);
			
			// If both dep and dest are in Europe/UK/Ireland, it's likely not oceanic
			// Regions: EG (UK), EI (Ireland), LF (France), ED (Germany), EB (Belgium), EH (Netherlands), 
			// EK (Denmark), ES (Sweden), EN (Norway), EF (Finland), LP (Portugal), LE (Spain), LI (Italy), LO (Austria), LS (Switzerland)
			bool depEuro = (depRegion == "EG" || depRegion == "EI" || depRegion == "LF" || depRegion == "ED" || depRegion == "EB" || 
						   depRegion == "EH" || depRegion == "EK" || depRegion == "ES" || depRegion == "EN" || depRegion == "EF" ||
						   depRegion == "LP" || depRegion == "LE" || depRegion == "LI" || depRegion == "LO" || depRegion == "LS");
			bool destEuro = (destRegion == "EG" || destRegion == "EI" || destRegion == "LF" || destRegion == "ED" || destRegion == "EB" || 
							destRegion == "EH" || destRegion == "EK" || destRegion == "ES" || destRegion == "EN" || destRegion == "EF" ||
							destRegion == "LP" || destRegion == "LE" || destRegion == "LI" || destRegion == "LO" || destRegion == "LS");

			if (depEuro && destEuro) {
				// Special exception: Iceland (BI) and Greenland (BG) are oceanic transit regions
				// But RYR14DH to EIKN is definitely domestic
				return false;
			}
		}

		// Directional boundaries
		int heading = target->GetPosition().GetReportedHeadingTrueNorth();
		bool isWestbound = GetAircraftDirection(heading);
		
		// Westbound: Nothing before 5W
		if (isWestbound && pos.m_Longitude > -5.0) return false;
		// Eastbound: Nothing before 58W
		if (!isWestbound && pos.m_Longitude < -58.0) return false;

		// Southbound: Nothing North of 62N
		if (heading >= 90 && heading <= 270) {
			if (pos.m_Latitude > 62.0) return false;
		}
		// Northbound: Nothing South of 42N
		if (heading < 90 || heading > 270) {
			if (pos.m_Latitude < 42.0) return false;
		}

		// 50nm Post-Oceanic check
		CFlightPlanExtractedRoute rte = fp.GetExtractedRoute();
		int lastOceanicIdx = -1;
		for (int i = 0; i < rte.GetPointsNumber(); i++) {
			string wp = rte.GetPointName(i);
			// For exit point check, we need to know the side. 
			// Westbound exit is Gander (true), Eastbound exit is Shanwick (false)
			if (CRoutesHelper::IsOceanicExitPoint(wp, isWestbound) || CUtils::IsCoord(wp)) {
				lastOceanicIdx = i;
			}
		}

		if (lastOceanicIdx != -1) {
			CPosition lastOceanicPos = rte.GetPointPosition(lastOceanicIdx);
			if (lastOceanicPos.m_Latitude != 0 || lastOceanicPos.m_Longitude != 0) {
				// Check if aircraft has passed it
				bool passed = false;
				if (isWestbound) {
					passed = pos.m_Longitude < lastOceanicPos.m_Longitude;
				}
				else {
					passed = pos.m_Longitude > lastOceanicPos.m_Longitude;
				}
				...
				if (passed) {
					double dist = pos.DistanceTo(lastOceanicPos);
					if (dist > 50.0) return false; // More than 50nm past exit
				}
			}
		}

		// General box (tightened to oceanic region)
		if (pos.m_Longitude > -65.0 && pos.m_Longitude < -2.0 && pos.m_Latitude > 40.0 && pos.m_Latitude < 68.0) {
			return true;
		}
		else {
			return false;
