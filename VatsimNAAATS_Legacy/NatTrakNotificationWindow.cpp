...
const int CNatTrakNotificationWindow::BTN_ACK = 600;
const int CNatTrakNotificationWindow::BTN_CLOSE = 601;
...
void CNatTrakNotificationWindow::RenderWindow(CDC* dc, Graphics* g, CRadarScreen* screen) {
	...
	dc->TextOutA(titleRect.left + 5, titleRect.top + 2, "NATTRAK REQUESTS");
	...
	for (const auto& callsign : notifications) {
		if (count >= 4) {
			dc->TextOutA(windowRect.left + 10, yOffset, "... more");
			break;
		}
		string text = callsign + " RCL PENDING";
		dc->TextOutA(windowRect.left + 10, yOffset, text.c_str());
