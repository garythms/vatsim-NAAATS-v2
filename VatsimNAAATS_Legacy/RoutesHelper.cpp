...
bool CRoutesHelper::GetRoute(CRadarScreen* screen, vector<CRoutePosition>* routeVector, string callsign, CAircraftFlightPlan* copy) {
	...
	if (fp->Route.empty() && !fp->RouteRaw.empty()) {
		...
		bool isWestbound = true;
		CRadarTarget target = screen->GetPlugIn()->RadarTargetSelect(callsign.c_str());
		if (target.IsValid()) {
			isWestbound = CUtils::GetAircraftDirection(target.GetPosition().GetReportedHeadingTrueNorth());
		}
		...
		// Find entry and exit indices in RouteRaw
			...
			for (int i = startIdx; i <= endIdx && i < fp->RouteRaw.size(); i++) {
				...
				bool isOceanic = IsOceanicEntryPoint(waypointName, isWestbound) || IsOceanicExitPoint(waypointName, isWestbound) || CUtils::IsCoord(waypointName);
	...
	if (fp->Route.size() == 0) {
		...
		// Get aircraft direction (default to true/Westbound if target invalid)
		bool isWestbound = true;
		if (target.IsValid()) {
			isWestbound = CUtils::GetAircraftDirection(target.GetPosition().GetReportedHeadingTrueNorth());
		} else {
		...
		for (int i = 0; i < fp->Route.size(); i++) {
			if (fp->Route[i].Name == "AIRCRAFT" || IsOceanicEntryPoint(fp->Route[i].Name, isWestbound) || CUtils::IsCoord(fp->Route[i].Name)) {
				startIndex = i;
				break;
			}
		}
		...
		for (int i = startIndex; i < fp->Route.size(); i++) {
			if (IsOceanicExitPoint(fp->Route[i].Name, isWestbound) || CUtils::IsCoord(fp->Route[i].Name)) {
				endIndex = i;
			}
		}
		...
		for (int idx = startIndex; idx <= endIndex; idx++) {
				...
				bool isOceanic = IsOceanicEntryPoint(fp->Route[idx].Name, isWestbound) || IsOceanicExitPoint(fp->Route[idx].Name, isWestbound) || CUtils::IsCoord(fp->Route[idx].Name);
...
void CRoutesHelper::InitialiseRoute(void* args) {
	...
	try {
		...
		if (!routeRaw.empty() && routeRaw.size() > 0) { // Get their route as per the route string
			...
			bool isWestbound = data->Direction;
			...
			for (int i = 0; i < tempRoute.size(); i++) {
				if (IsOceanicEntryPoint(tempRoute[i].Name, isWestbound)) { entryIdx = i; break; }
			}
			for (int i = tempRoute.size() - 1; i >= 0; i--) {
				if (IsOceanicExitPoint(tempRoute[i].Name, isWestbound)) { exitIdx = i; break; }
			}
...
bool CRoutesHelper::IsOceanicEntryPoint(string pointName, bool isWestbound) {
	// 1. Check static lists
	if (CUtils::IsEntryPoint(pointName, isWestbound)) return true;
...
bool CRoutesHelper::IsOceanicExitPoint(string pointName, bool isWestbound) {
	// 1. Check static lists
	if (CUtils::IsExitPoint(pointName, isWestbound)) return true;
...
string CRoutesHelper::OnNatTrack(CRadarScreen* screen, string callsign, string routeString, bool disableEuroScopeFetch) {
	try {
		...
		CLogger::Log(CLogType::ERR, "An error occurred in OnNatTrack. Details: " + string(ex.what()), "CRoutesHelper::OnNatTrack");
		return "";
	}
}

bool CRoutesHelper::IsOceanicRoute(CRadarScreen* screen, CFlightPlan fp) {
	...
	// 1. Check if on a track
	if (OnNatTrack(screen, fp.GetCallsign(), route, true) != "") return true;

	// 2. Check route for coordinates or oceanic entry points
	// Normalize route
	string normRoute = route;
	for (auto& c : normRoute) c = toupper((unsigned char)c);
	
	// Quick check for coordinates (common in NAT)
	if (normRoute.find("N0") != string::npos || normRoute.find("S0") != string::npos || normRoute.find("/") != string::npos) {
		// Verify with a more robust check if needed, but these are very likely NAT coords
		// Split and check each item for better accuracy
		stringstream ss(normRoute);
		string item;
		while (ss >> item) {
			if (CUtils::IsCoord(item)) return true;
		}
	}

	// 3. Check for entry/exit points
	stringstream ss(normRoute);
	string item;
	while (ss >> item) {
		if (item.empty() || item == "DCT") continue;

		// Check both directions for relevance
		if (IsOceanicEntryPoint(item, true) || IsOceanicEntryPoint(item, false) ||
			IsOceanicExitPoint(item, true) || IsOceanicExitPoint(item, false)) {
			return true;
		}
	}
