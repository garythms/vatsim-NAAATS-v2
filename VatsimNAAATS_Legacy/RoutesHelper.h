...
class CRoutesHelper
{
	public:
		...
		static bool IsOceanicEntryPoint(string pointName, bool isWestbound);
		static bool IsOceanicExitPoint(string pointName, bool isWestbound);

		// Is on a NAT track
		static string OnNatTrack(CRadarScreen* screen, string callsign, string routeString = "", bool disableEuroScopeFetch = false);

		// Check if a flight plan is relevant to the oceanic region (contains entry/exit fixes or coordinates)
		static bool IsOceanicRoute(CRadarScreen* screen, CFlightPlan fp);
};
