...
void CAcTargets::RenderTarget(Graphics* g, CDC* dc, CRadarScreen* screen, CRadarTarget* target, bool tagsOn, map<int, CWinButton>* toggleData, bool halo, bool ptl, CSTCAStatus* status) {
	...
	// Change targets depending on NATTrak status
	...
	if (showAirplane) {	
		...
		// NATTrak PENDING - Diamond with line (not yet cleared)
		...
		// NATTrak UNKNOWN - Asterisk (not in NATTrak system)
...
POINT CAcTargets::RenderTag(Graphics* g, CDC* dc, CRadarScreen* screen, CRadarTarget* target, pair<bool, POINT>* tagPosition, bool direction, CSTCAStatus* status, string asel) {	
	...
	try { // Temporary
		...
		bool shouldRenderTag = true;  // Used for transfer timeout

		

		// Check for transferred aircraft (yellow tag that disappears after 5min or 50nm)

		CAircraftFlightPlan* fpPtr = CDataHandler::GetFlightData(acFP.GetCallsign());

		if (fpPtr != nullptr && fpPtr->IsTransferred) {

			time_t now = time(0);

			double elapsedMinutes = difftime(now, fpPtr->TransferTime) / 60.0;

			

			// Calculate distance from transfer position

			CPosition transferPos;

			transferPos.m_Latitude = fpPtr->TransferPositionLat;

			transferPos.m_Longitude = fpPtr->TransferPositionLon;

			double distanceNm = position.DistanceTo(transferPos);

			

			// Check if 10 minutes OR 100nm has passed
			if (elapsedMinutes >= 10.0 || distanceNm >= 100.0) {

				// Don't render tag at all - it should disappear

				shouldRenderTag = false;

			} else {

				// Full yellow tag for transferred aircraft
		...
		else if (status->ConflictStatus == CConflictStatus::CRITICAL) {
		...
		// Flashing logic for coordination (flash between normal color and white)

		if (fpPtr != nullptr && fpPtr->IsCoordinationFlashing && shouldRenderTag) {

			// Flash every 500ms

			if (twoSecT >= 0.5 && twoSecT < 1.0) {

				// Keep original color

			} else if (twoSecT >= 1.0 && twoSecT < 1.5) {

				textColour = TextWhite.ToCOLORREF();

			} else if (twoSecT >= 1.5 && twoSecT < 2.0) {
		...
		// Early return if tag should not be rendered (transferred and timed out)
		...
		// Callsign - add brackets if connected to CPDLC
		...
		CHoppieClient* hoppieClient = CCPDLCWindow::GetHoppieClient();

		if (hoppieClient != nullptr && hoppieClient->IsConnected()) {

			CConnectedAircraft* cpdlcAc = hoppieClient->GetAircraft(text);

			if (cpdlcAc != nullptr && cpdlcAc->LoggedOn) {
		...
		// Add climb/descend arrow based on vertical speed

		int verticalRate = target->GetVerticalSpeed();

		if (verticalRate > 300) {

			// Climbing - add up arrow

			text += "^";

		} else if (verticalRate < -300) {

			// Descending - add down arrow
